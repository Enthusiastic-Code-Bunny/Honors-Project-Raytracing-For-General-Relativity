...
def backgroundImage(phi, theta, background_img):
    """
    This module defines a CUDA device function that samples color data
    ...
    --------
        (r, g, b) : tuple of floats
            The red, green, and blue color values sampled from
            the corresponding pixel of the image.
    ...
    # Samples the color found at that little square in the picture 
    # (note: images index rows first, that's why y then x)
    r = background_img[y, x, 0]
    g = background_img[y, x, 1]
    b = background_img[y, x, 2]
