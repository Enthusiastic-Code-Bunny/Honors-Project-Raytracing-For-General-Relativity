...
from runIntegration import runIntegration
...
def main():
    ...
    impact parameters, runs geodesic integrations, and assembles
    ...
    4. Integrate null geodesics in parallel
    5. Construct the final rendered image and save it to disk.

    Notes
    -----
    * The computation is multi-threaded for the geodesic integration step.
    * The field of view (FOV) is specified in radians.
    * The observer is placed at a fixed radius 'r0' from the black hole.
    '''
    totalTime = 0.0            # Adds up the total simulation time over multiple iterations
    ...
    for _ in range(iters):
        ...
        phi_out, r_out = runIntegration(numRays, M, b_vals, r0)

        # Create final image from the integration results
        print("Creating final image...")
        image = createImage(screenWidth, screenHeight, numRays, phi_out, r_out
                            , alphaVals, M)
